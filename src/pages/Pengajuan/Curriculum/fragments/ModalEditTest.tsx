...
export default function ModalEditTest({
  ...
  const puthandler = async (values: any, actions: any) => {
    let val = await putTest(payload.submissionId, values);
  ...
  return (
    ...
    >
      ...
      <ModalContent maxW={{ base: "87.5vw", sm: "xl" }} py={4} borderRadius="3xl">
        <ModalBody>
          <Heading fontSize={20}>Edit Penyelarasan Kurikulum</Heading>
          <FetchMessageEdit condition={fetchConditon} message={message} />
          <Formik
            initialValues={{
              alignmentDate: payload.alignmentDate.split('T')[0],
              subject: payload.subject,
              agencyName: payload.agencyName,
              duration: {
                value: payload.duration.value,
                period: payload.duration.period
              },
